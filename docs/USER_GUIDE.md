...
The **Upload Deloitte PDF** tab parses the weekly report first. Review the page and row classifications before applying the changes. The **Update from ManageEngine** tab shows the automatic API sync state and its latest counts; administrators can also run it immediately. Each API sync refreshes existing SRs and automatically adds previously untracked, active requests in the `Oracle ERP` category. New closed tickets and other categories are ignored. For quicker access, administrators can use **Sync SRs from ManageEngine** directly on the Service Requests page; the table, status tiles, and filter choices refresh when the sync completes. The CSV parse-and-apply workflow remains available as a manual fallback.
