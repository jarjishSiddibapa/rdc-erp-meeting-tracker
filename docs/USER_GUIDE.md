...
The **Upload Deloitte PDF** tab parses the weekly report first. Review the page and row classifications before applying the changes. Identical repeated Request IDs are collapsed automatically. If repeated copies disagree on the table, subject, comment, or Expected Closure Date, the SR is shown as **Conflict - skipped** and cannot be applied until it is verified manually; the importer never guesses between conflicting dates. The **Update from ManageEngine** tab shows the automatic API sync state and its latest counts; administrators can also run it immediately. Each API sync refreshes only SR numbers already present in this tracker. Untracked ManageEngine requests are ignored, regardless of category or status. For quicker access, administrators can use **Sync SRs from ManageEngine** directly on the Service Requests page; the table, status tiles, and filter choices refresh when the sync completes. The CSV parse-and-apply workflow remains available as an update-only manual fallback.
